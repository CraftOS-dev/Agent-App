...
|---|---|---|
| [cli/](cli/) | `agent-app` | Two binaries, one package (spec 5.1). `agent-app`: `scaffold`, `validate`, `toolkit-sync`, `adapter-sync`, `serve`/`stop`, `list`, `global`, `skills`, `walk-verify`, plus safe-evolve `dev`/`promote`/`backup`/`restore`. `a2app`: the operate client — `identity`, `data`, `ops`, `run`, `whoami`, `context`, `tasks`, `events`. Exit codes 0/1/2/3, machine-readable stdout. |
