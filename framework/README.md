...
| [host/](host/) | `@craftos/agent-app-host` | Optional reference host: launches apps via the manifest `pipeline` block, polls health, runs adapter-sync at every launch, and may add host features (watchdog, build supervision, receipts voice). **A bare agent plus a browser is a complete environment — nothing here is required for compliance.** |
