...
| Export | What it does |
|---|---|
| `a2appTools(cliBin?)` | The 12 build+operate tools (`agent_app_describe`, `_list`, `_get`, `_create`, `_update`, `_delete`, `_operations`, `_run_operation`, `_poll_tasks`, `_build`, `_validate`, `_walk_verify`). Each shells a real `a2app` verb. |
...
The engine drives the real CLI end-to-end: `agent_app_build` scaffolds an app and `agent_app_validate` runs the gate (both exit 0). Every other tool uses the same `runA2App` path.
