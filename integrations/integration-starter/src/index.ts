 ...
 * `a2app` CLI as a set of agent tools (so the agent can build/operate an Agent
...
export interface PluginOptions {
  ...
  skillsDir?: string;
  /** the `a2app` binary or entry to shell. Default "a2app". */
  cliBin?: string;
}

/**
 * Run the a2app CLI and resolve a structured result. Never uses a shell (field
 ...
 * do through these, because each one shells the real `a2app` CLI.
 */
export function a2appTools(cliBin = "a2app"): HarnessTool[] {
  return [
    {
      ...
      handler: (a) => runA2App(cliBin, ["data", String(a.dir), "schema"]),
    },
    {
      name: "agent_app_list",
    ...
    },
    {
      ...
      handler: (a) => runA2App(cliBin, ["data", String(a.dir), String(a.entity), "get", String(a.id)]),
    },
    {
      ...
      handler: (a) => runA2App(cliBin, ["data", String(a.dir), String(a.entity), "create", "--json", JSON.stringify(fieldsOf(a, "fields"))]),
    },
    {
      ...
      handler: (a) => runA2App(cliBin, ["data", String(a.dir), String(a.entity), "update", String(a.id), "--json", JSON.stringify(fieldsOf(a, "fields"))]),
    },
    {
      name: "agent_app_delete",
      description: "Delete a record by id.",
      parameters: obj({ dir: DIR, entity: ENTITY, id: { type: "string" } }, ["dir", "entity", "id"]),
      handler: (a) => runA2App(cliBin, ["data", String(a.dir), String(a.entity), "delete", String(a.id)]),
    },
    {
      name: "agent_app_operations",
      description: "List the app's declared operations (its agent verbs).",
      parameters: obj({ dir: DIR }, ["dir"]),
      handler: (a) => runA2App(cliBin, ["ops", String(a.dir)]),
    },
    {
    ...
    },
    {
      ...
      handler: (a) => runA2App(cliBin, a.status != null ? ["tasks", String(a.dir), "--status", String(a.status)] : ["tasks", String(a.dir)]),
    },
    {
    ...
    },
    {
      ...
      handler: (a) => runA2App(cliBin, a.noBuild ? ["validate", String(a.dir), "--no-build"] : ["validate", String(a.dir)]),
    },
    {
      name: "agent_app_walk_verify",
      description: "Independently verify a running app against its requirements (walk-verify).",
      parameters: obj({ dir: DIR }, ["dir"]),
      handler: (a) => runA2App(cliBin, ["walk-verify", String(a.dir)]),
    },
  ];
}
...
export function registerA2AppPlugin(ctx: HarnessContext, opts: PluginOptions = {}): HarnessTool[] {
  ...
  ctx.registerCommand?.({ name: "agent-app", run: async (argv) => (await runA2App(cliBin, argv)).code });
  return tools;
}
