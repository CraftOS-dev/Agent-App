...
A real OpenClaw plugin (`definePluginEntry`) that registers agent tools for building and operating Agent Apps, an `openclaw agent-app …` CLI passthrough, and a Control-UI tab for a launched app. Every tool shells a framework CLI through [`@a2app/integration-starter`](../integration-starter/).
