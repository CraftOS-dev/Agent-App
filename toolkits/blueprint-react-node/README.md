...
A Node + browser Agent App blueprint. `a2app create --blueprint blueprint-react-node` scaffolds a running to-do app: a Node backend on the built-in `http` module with a JSON-file store, a dependency-free SPA View, and the A2App adapter (`@a2app/adapter-core`) mounted as embedded middleware. Built from [../blueprint-base/](../blueprint-base/); vendors the reference [kit](../kit/).
